...
UsbCanConnection::Start() {
	...
	int ret = pthread_create(&listenerThread, NULL,
	                         (void* (*)(void*)) & UsbCanConnection::_thread, this);
	if (ret != 0) {
...
void
UsbCanConnection::SetReceiver(CanListener* cl) {
	listener = cl;
};
int
UsbCanConnection::initSocket() {
	if (socketfd > 0) {
		close(socketfd);
	}
...
int
UsbCanConnection::SendExCanMsg(unsigned int canid, unsigned char* data, int len) {
	if (len < 0 || len > 8) {
...
int
UsbCanConnection::SendCanMsg(unsigned short canid, unsigned char* data, int len) {
	if (len < 0 || len > 8) {
		printf("Data length is out of range in SendCanMsg: %d\n", len);
		return -1;
	}
...
int
UsbCanConnection::SendCanRtr(unsigned short canid) {
	tx_frame.can_id = canid;
...
int
UsbCanConnection::SendExCanRtr(unsigned int canid) {
	tx_frame.can_id = canid;
	tx_frame.can_id |= CAN_RTR_FLAG;
	tx_frame.can_id |= CAN_EFF_FLAG;
	tx_frame.can_dlc = 0;
...
int
UsbCanConnection::WaitForCanMsg(unsigned int msgid, unsigned char* buffer, int* len,
                                int timeout) {
	waitOnId = msgid;
	int i = 0;
...
void*
UsbCanConnection::_thread(void* This) {
